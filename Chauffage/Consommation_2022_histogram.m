...
E_annuelle_simulation = sum(E_mensuelle);

chauffage_reel_2022 = 14.89 / 100 * [455000, 378000, 331000, 229000, 8000, 0, 0, 0, 0, 0, 477000, 300000]; % kWh

E_reelle = chauffage_reel_2022 / 1000; % Energie (MWh)

E_annuelle_reelle = sum(E_reelle);

% Create a bar plot for annual consumption
figure;
b = bar([1 2], [E_annuelle_reelle, E_annuelle_simulation], 'FaceColor', 'flat');
b.CData(1, :) = [0 0.4470 0.7410]; % Blue color for real consumption
b.CData(2, :) = [0.8500 0.3250 0.0980]; % Orange color for simulated consumption
